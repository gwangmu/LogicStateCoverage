...
fuzzing campaign. Logic states represent more \emph{interesting} behaviors
(i.e., relating to unknown abnormal behaviors) in a finer granularity, meaning
that observing more interesting behaviors is reliably reflected to larger
coverage. 
...
mutation base is highly repetitive, as fuzzers commonly incorporate corpus
minimization \cite{todo} and branch hit count buckets \cite{todo}.
%
The rationales are: (i) if a fewer repetition was normal, more repetition is
also likely normal (i.e., unlikely \emph{abnormal}) and (ii) even if it was
abnormal, it may correspond to pre-observed less-repetition counterparts (i.e.,
unlikely \emph{unknown}).
