...
\begin{abstract}
  ...
  branches during one execution, where its coverage is the count of
  ...
  % 
  We implemented logic state coverage using a bloom filter and performed a
  preliminary evaluation with AFL++ and XMLLint.
  % reevaluated \textsc{FuzzBench} and indecisive-to-subjective fuzzer
  % comparisons in hypervisor and grammar fuzzing. 
...
\begin{itemize}
  ...
  \item \textbf{RQ3} Can logic state coverage distinguish the quality of the
    fuzzers indistinguishable from conventional edge coverage?
\end{itemize}
