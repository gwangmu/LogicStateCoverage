...
\begin{abstract}
  ...
  more \emph{interesting} (i.e., relating to unknown abnormal) behaviors. This
  suggests that the number of observed interesting behaviors should directly
  indicate how well the fuzzer achieved its aim (i.e., the quality). 
...
large number of branch hit when it gets larger.
%
...
best accuracy among all alternatives 
\cite{harmouch2017cardinality}. 
...
\emph{bloom filter}, representing a set of observed logic states, by setting the
hashed indices (\autoref{s:design:up}).  Finally, the measurement periodically
