...
behaviors contain more branch repetitions, resulting in a smaller (or larger)
logic state coverage as it observes more uninteresting (or interesting) program
behaviors.
...
gridded up by the different hit counts of the branch $b_1$, $b_2$, and $b_3$.
...
(i.e., $m(b_1)=m(b_2)=m(b_3)=0$) falls into the \cbox{black} logic state, while
all program behaviors that hit only $b_1$ (i.e., $m(b_1)>1$ and
$m(b_2)=m(b_3)=0$) fall into the \cbox{red} logic state.
...
space (i.e., \cbox{black} $\rightarrow$ \cbox{red}/\cbox{green}/\cbox{blue}
...
%
This results in repetitive (i.e., less interesting) program states coalesced to
fewer logic states, suppressing logic state coverage as it observes
uninteresting behaviors more.
