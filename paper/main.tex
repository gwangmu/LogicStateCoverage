...
%\autoref{f:repr} reveals two important properties of logic states. First, a
%logic state distinguishes interesting program behaviors with smaller branch hit
%counts in a finer granularity. For example, a logic state represents
%exponentially fewer program behaviors as $m(b_1)$ and $m(b_2)$ approach to 1.
%Second, a logic state coalesces less interesting program behaviors that require
%high branch hit counts or exhibit specific branch ordering, avoiding
%over-representation of them. For example, all 11,950 program behaviors
%exhibiting different branch ordering with $4 \le m(A) \le 7$ and $4 \le m(B) \le
%7$ are lumped into \emph{one} logic state.
