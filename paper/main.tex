...
%
The legitimacy of logic state coverage hinges on the current conventional
understanding of interesting program behaviors in \emph{untargeted} fuzzing,
namely the behaviors with less repetition. However, recent findings 
\cite{regression,risk,root-causes-age} suggest that various code characteristics
(e.g., the age of code) make some code likelier to cause bugs,
which makes the program behaviors executing such a portion of code have a higher
expectation to reveal unknown abnormal behaviors (i.e., more interesting).
%
\emph{Targeted} fuzzing \cite{regression,directed,risk} attempts to capitalize
the findings by prioritizing such code, while logic state coverage would
under-estimate their quality by only considering less repetition as more
interesting. 

For a quality metric that encompasses general (i.e., both untargeted and
targeted) fuzzing, future research needs to address two issues around
interestingness. First, it should refine the interestingness of program
behaviors. Not only should it systemize the factors that decide interestingness,
but also it should assess which factor affects \emph{more} than others by
\emph{how much}.  Second, it should incorporate such refined interestingness
into the quality metric. If it would follow the philisophy of logic state
coverage (i.e., more uninteresting behavior in fewer logic states),
\emph{grouping} uninteresting branches can be an approach as it makes the
program behaviors with uninteresting branches occupy fewer logic states.
