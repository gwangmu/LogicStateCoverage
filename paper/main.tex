...
\title{A Comparative Quality Metric for Fuzzing with Logic State Coverage}
\author{
  {\rm Gwangmu Lee} \\ {EPFL}
}
...
\begin{abstract}
  ...
  aims to reduce the number of unknown \emph{abnormal} behaviors by observing
  ...
  count observed interesting behaviors. A logic state distinguishes interesting
  behaviors in a finer granularity than the less interesting ones, making the
  amount of logic state coverage (i.e., observed logic states) directly
...
is defined as the \emph{individual count} of all observed distinct logic states.
One logic state represents \emph{fewer} behaviors if they are \emph{more
interesting} (i.e., likely relating to unknown abnormal behaviors), meaning that
observing more interesting behaviors is strongly reflected on larger logic state
coverage. 
...
\pp{Special case: single-thread programs}
%
...
%Notice that exceptions may be implicit, unlike branches that are always explicit
%in the program; exceptions such as assertions are explicitly specified in
%the program, but others such as divide-by-zero (i.e., $\mathrm{divisor}=0$) or
%buffer-overflow (i.e., $\mathrm{offset} \ge \mathrm{size}$) are implicit and
...
behaviors are a subset of the \emph{new} behaviors \emph{not equivalent} to any
observed behaviors. In particular, an abnormal behavior is \emph{unknown} if it
is not equivalent to any observed abnormal behaviors (e.g., a unique root
cause). Formally, if $\mathrm{U}$ and $\mathrm{N}$ are the sets of
\underline{\textbf{u}}nknown and \underline{\textbf{n}}ew behaviors,
respectively, $\mathrm{U} \subset \mathrm{N}$.

\pp{Formal justification}
%
The approach of untargeted fuzzing can be described formally as follows. Upon
executing a mutated input, let $P(\mathrm{N})$ be the probability of observing a
\underline{\textbf{n}}ew behavior as a result of execution. Then, the
probability of discovering \underline{\textbf{u}}nknown
\underline{\textbf{a}}bnormal behavior $P(\mathrm{UA}) := P(\mathrm{U} \cap
\mathrm{A})$ is
%
\begin{align}
  P(\mathrm{UA}) 
      &= P(\mathrm{N} \cap \mathrm{UA}) \tag{by $\mathrm{U} \subset \mathrm{N}$} \\
      &= P(\mathrm{N}) \times P(\mathrm{UA} \mid \mathrm{N}) \tag{by chain rule}. 
\end{align}

Here, the second term $P(\mathrm{UA} \mid \mathrm{N})$ is the expectation of
...
untargeted fuzzing attempts to (i) increase $P(\mathrm{N})$ to investigate more
program behaviors while (ii) keeping the interestingness $P(\mathrm{UA} \mid
\mathrm{N})$ as high as possible.
%
...
%
As the interestingness $P(\mathrm{UA} \mid \mathrm{N})$ is the expectation of an
\emph{unobserved} behavior, there is no direct way to measure interestingness. 
%
However, the practice of untargeted fuzzing suggests that
the resulting program behavior is unlikely to be \emph{unknown abnormal} if the
mutation base is highly repetitive, as it commonly incorporates corpus
minimization \cite{todo} and branch hit count buckets \cite{todo}.
%
The rationales are: (i) abnormal behaviors might have been already observed with
fewer branch repetitions (i.e., unlikely \emph{abnormal}) and (ii) if so, it may
correspond to pre-observed less-repetition counterparts (i.e., unlikely
\emph{unknown}).
...
\begin{figure}[t]
  ...
  of the branch $b$. Dotted lines demarcate whether the corresponding branch is
  hit or not (i.e., presence in a logic state).}
...
count of the branch $b$. For example, there are two possible program behaviors,
$b_2$ after $b_1$ or $b_1$ after $b_2$, when $(m(b_1), m(b_2))=(1, 1)$. 
%Notice that the back edge $b_3$ is always taken $m(b_1)+m(b_2)$ times by the shape of the graph.
%
...
%the beginning of a new power-of-2 branch hit count
...
As shown, the program behaviors with more repetitive branches are grouped with
more program behaviors as one logic state.  For example, a program behavior
having only one repetitive branch (either $b_1$ or $b_2$) falls into the slim
logic states adjacent to axis (bottom right and top left), while a behavior
having two repetitive branches (both $b_1$ and $b_2$) falls into the large logic
state (top right). This results in observing repetitive (i.e., less interesting)
program states represented by fewer logic states, leading to smaller logic state
coverage. 
...
%, whereby $P(\mathrm{new} \cap
%\mathrm{abnormal})$ then majorly depends on how high $P(\mathrm{new})$ is;
